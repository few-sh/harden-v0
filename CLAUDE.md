...
- Per-task last-seen SHA at `<task_output_dir>/pool_sha.txt`. **In batch mode**, every fresh task's cursor is pre-seeded to the bootstrap SHA (in `batch.harden_batch`) so iter 0 runs the hacker normally instead of burning a fixer turn integrating the seed commit. Single-task runs of `harden_task` skip this seeding — `PoolCursor` defaults to `None`, which makes iter 0 a skip-hacker pool-sync iteration. Persisted only at commit points (fix validated, pool-sync no-op, or legitimate flag) — an iteration that crashes does NOT silently mark pool commits as seen.
- Iteration branches:
