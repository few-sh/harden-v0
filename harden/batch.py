...
async def harden_batch(config: BatchHardenConfig) -> list[dict]:
    ...
    logger.info(
        "Batch hardening: %d tasks, max %d concurrent containers, oracle=%s kernelbench_mode=%s",
        total, config.max_concurrent_containers, config.oracle, config.kernelbench_mode,
    )
